...
function badge(value) {
  return `<span class="badge ${String(value).toLowerCase()}">${String(value).replaceAll('_', ' ')}</span>`;
}
...
async function loadOverview() {
  try {
    ...
    $('#total-orders').textContent =
      data.work_orders_total;

    $('#verified-potholes').textContent =
      status.verified || 0;

    const values = data.potholes_by_severity;
    ...
    $('#connection-label').textContent = 'API online';

  } catch (error) {
    $('#connection-label').textContent = 'API unavailable';
  }
}
      ...
      .map(
        c => `
          <tr>
            ...
            <td>
              ${badge(c.active ? 'active' : 'inactive')}
            </td>
          </tr>
        `
      )
...
$('#detect-button').addEventListener(
  ...
  async () => {
    ...
    if (!file) {
      return toast('Choose an image first.');
    }
